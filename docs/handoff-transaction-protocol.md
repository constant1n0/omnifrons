...
| HTP-001-R7 | A second claim observed for the same handoff from a different receiver, before RSP-001 defines compare-and-set claim records, MUST render `conflicted` and MUST require human review; Omnifrons MUST NOT resolve it by last-writer-wins. |
...
| D5 | How are racing claims resolved before RSP-001 defines compare-and-set claim records? | Last-writer-wins; first-writer-wins; `conflicted` plus human review | `conflicted` plus human review; revisit once RSP-001 delivers compare-and-set claim records |
