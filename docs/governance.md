...
| Advice kind | The `advice_kind` field on an approval record: one of `human-security-reviewer`, `human-legal-counsel`, `human-independent-reviewer`, `automated-lens`, or `none`, naming who or what gave the advice an approval relies on, or stating that it relies on none (see Review evidence and retention). |
...
The "Today" column states the interim reality as fact rather than concealing it, following the same discipline as the ADR convention's interim glossary: the Project Owner, Project Maintainer, Decision Owner, Compatibility Owner, Release Approver, and sole Root Key Holder roles above are held by one person; Security Reviewer, Legal Counsel, and Independent Reviewer are unfilled or filled only by recorded advice; App Registry Curator is unfilled and inactive until UTA-001's curated registry ships; Asset Policy Owner is unfilled and exercised by the Project Owner as fallback. Conflicts of interest and combined roles are recorded in the decision evidence, per the ADR convention.
...
| Asset Policy Owner | Unassigned (fallback: Project Owner) | — | Role active since HAP-001's acceptance on 2026-09-08; the Project Owner exercises it until it is filled |
...
An approval record's `advice_kind` field takes exactly one value: `human-security-reviewer` (naming the reviewer's register handle and carrying their signature), `human-legal-counsel` (naming the counsel's register handle and the identifier of the advice document, signed with the counsel's registered key where one exists and otherwise filed in the private evidence store and referenced by identifier), `human-independent-reviewer`, `automated-lens` (naming the tool identifier), or `none` (the approval relies on no advice; permitted only for a transition that no gate item ties to advice, recorded alongside the self-approval flag whenever the accountable and approver roles coincide, and never for the Alpha → Beta items GOV-001-R4 names). Security Reviewer advice MUST be signed by that human reviewer's own registered key; an automated lens's ledger can never carry the Security Reviewer or Legal Counsel role, labelled or not (GOV-001-R14; open decision D7 asks whether an automated lens's advice separately counts as Independent Reviewer advice).
...
| GOV-001-R14 | A review performed by an automated review lens MUST be labelled as automated advice in its ledger and recorded with `advice_kind: automated-lens` naming the tool identifier; it MUST NOT be recorded as Security Reviewer or Legal Counsel advice under any circumstance. Security Reviewer advice MUST carry `advice_kind: human-security-reviewer`, the reviewer's register handle, and their signature; Legal Counsel advice MUST carry `advice_kind: human-legal-counsel`, the counsel's register handle, and the advice document identifier; a record with `advice_kind: none` MUST NOT satisfy any gate item that requires advice. |
...
- The frozen-sections list (GOV-001-R16) was first exercised on 2026-09-05: a `Proposed` revision of the target architecture's planned assurance artifacts table, adding pointer-only text to the VP-001 and VOC-001 rows, was accepted with the signed tag `accept/target-architecture/2026-09-05` and its approvals row.
