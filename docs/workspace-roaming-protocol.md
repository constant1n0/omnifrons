...
- **The migration and recovery plan** (MRP-001, not yet drafted) owns backup formats, restore epochs, tombstone formats, and the upgrade graph. The cutover checklist below names the steps and gates a Git Sync to Cloud migration must pass; it points at MRP-001 for the artifact formats those steps produce rather than defining them here.
...
- Migration and recovery plan (MRP-001, not yet drafted) — backup, restore-epoch, and tombstone formats the cutover checklist depends on.
