...
Adopted from VOC-001's proposed threat-model additions: the retained transcript row carries a residual risk mirroring SEC-3's own disclosure above — `<private>` stripping and tombstone deletion remove what they are told to, not what a reviewer failed to tag or a restore failed to exclude, and a transcript already synchronized before deletion has already reached every device and Cloud target the namespace replicates to (voice-interaction-contract.md § Proposed threat-model additions ("transcript row carries a residual")).
...
| PRC-6 | A2 a harness process holds a descriptor that belongs to another launch — that launch's pseudo-terminal, or the sealed in-memory copy of its executable — inherited across `fork`/`exec` because it was briefly not close-on-exec while a concurrent launch forked | Through another launch's terminal a harness can read that harness's output, type input into it — text the other harness takes for its user's — or keep it open and delay the end of its output capture; isolation between harnesses fails with no approval involved. A sealed executable copy is read-only | Every descriptor ProcessSupervisor opens is close-on-exec, and the two moments one is not — between opening a pseudo-terminal and marking it, and while a sealed copy is made inheritable for its own `exec` — are serialized against every `fork` the supervisor makes by one process-wide lock (ADR-0002) | The lock covers only the forks ProcessSupervisor itself makes: a spawn site added elsewhere in the same process reopens the window unless it takes the same lock. Descriptors the product's own environment handed it as inheritable pass through to every child; the supervisor does not close them. The evidence is two concurrent-launch tests — the pseudo-terminal window on Linux and macOS, the sealed-copy window on Linux, the only platform where it exists — and a passing run shows no leak in that run, not that a window is closed; Windows handle inheritance is unexamined |

## Prompt injection

Sources: repository files; content a harness fetches; feed events; startup briefs; memory; handoff task text; third-party app output; terminal or command output that re-enters a harness's own context; a transcript that is misheard, spoken back, or re-entered as instruction-like content reaching a harness (the spoken-misheard-re-entered round trip, adopted from VOC-001's proposed threat-model additions, voice-interaction-contract.md § Proposed threat-model additions ("Entry adopted into the prompt-injection")); file and path names; and commit messages or branch names. Posture:
...
- A descriptor-inheritance test: under concurrent launches no child holds a terminal, or a sealed executable copy, beyond what a baseline launch already shows the product's own environment made inheritable, within the descriptor range the reporting fixture scans (PRC-6).
