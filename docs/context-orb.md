...
**Interactive graph view (takeover surface).** Activating the idle orb plays a three-beat transition — particle implosion (~0.5 s, ease-in), hard white flash (~0.1 s), ease-out fade (~0.4 s) — into a full-screen concentric graph on the warm near-black ground. The orb's interior stays clean — only the flake structure. Between the routines band and the applications band, a honeycomb at application-cell size **fades in and continues outward, covering the entire dashboard**: its cells are the **application slot grid** — an empty cell is an unlit slot, an occupied cell lights up as the application. Knowledge is the figure; the app fabric is the ground it floats on. From the center outward:
