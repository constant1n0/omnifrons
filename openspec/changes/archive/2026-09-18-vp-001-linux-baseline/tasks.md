...
- [x] 4.1 **Authorization granted and exercised.** The user explicitly authorized exactly one `.github/workflows/tauri-build.yml` `workflow_dispatch` (`vp001_scenario: true`); it ran once as run `35252892166` on `ubuntu-24.04`, head commit `ab5eda8` on `main`, conclusion `success`. This apply batch never re-dispatched the workflow — the run and its facts were verified from the parent's supplied evidence and the run's own downloaded logs/transcripts/digest files.
- [x] 4.0 Wire the full VP-S6 scenario into `.github/workflows/tauri-build.yml` behind an opt-in `vp001_scenario` dispatch input (default off): build `tools/vp-s6-agent`, run `vp-s6-linux.sh --mode=scenario` on the pinned runner against the digested artifact, and upload the feasibility and scenario transcripts even when a gate fails. Found at the slice 3b/4 boundary: the workflow only ran `--mode=feasibility-check`, and VP-001 admits a scenario result only from the baseline it ran on, so a local scenario run against a CI-built artifact would not be admissible.
- [x] 4.2 **Depends on 4.1.** Trigger the authorized run on `ubuntu-24.04` with `vp001_scenario: true`; capture the retained artifact, its SHA-256 digest, Slice 1's recorded baseline facts, and both transcripts. Done: AppImage digest `d7e1f946ba6bea057626c7d9b5f1986584e6adee58cbbec273085b1c21c94f68` (sibling `.deb`/`.rpm` digests also recorded), baseline facts (`os_build`, `webview_runtime`, MSRV/toolchain, runner image `ubuntu-24.04` version `20260907.300.1`/provisioner `20260828.587`/runner `2.337.0` transcribed from the run's own "Set up job" log), and both the feasibility and VP-S6 transcripts, all read from the downloaded run artifacts.
- [x] 4.3 **Depends on 4.1/4.2 and the Slice 3a gate outcome.** If 3a's F1–F3 (and AV1/AV2) passed, run the full VP-S6 scenario (Slice 3b) against this run's retained artifact. If 3a filed the disclosed-uncertain branch, skip straight to 4.5's uncertain branch — never force a scenario run once feasibility already failed. Done: 3a's gate passed (see the 2026-09-17 addendum under Phase 3a), so the workflow ran the full VP-S6 scenario per 4.0's wiring. AV1/AV2 matched, F1 session created, but the scenario itself hit `blocker=workspace-chooser-timeout` before Start was reachable — no fixture process was ever spawned. Recorded honestly on the row (4.5), not treated as a 3a-style disclosed-uncertain skip.
- [x] 4.4 `docs/evidence/VP-001/baselines.md` — create: append one baseline record with every VP-001-R1 field (OS build, architecture, WebView/runtime version, packaging substrate, assistive-technology product authored as `none exercised — VP-S19 out of scope`, test date), sourced from 4.2. Validate with `cargo test --workspace` before proceeding. Done: `VP-001-BASE-01`, plus the pinned runner/toolchain facts as extra fields. `cargo test --workspace` green.
- [x] 4.5 `docs/evidence/VP-001/records.md` — create: append one VP-S6 row referencing that `baseline_id`, `build_channel: packaged-ci`, `build_channel_digest`, `exercised_artifact_digest`, `variant_scan`, mechanism (`process-group killpg(SIGTERM)`) and fallback (`killpg(SIGKILL) escalation`) as two distinct observations, and the honest outcome:
  ...
  Validate against Slice 2's V1–V8 before considering the row filed. Done: `VP-001-VP-S6-01` — `result: uncertain`, `observed_state: orphan-risk` (the only value `derive()` admits from this run's recorded observations: `identity_gated=true`, all five stop/enumeration observations `false`, `blocker=workspace-chooser-timeout`). Mechanism and fallback recorded as two distinct not-exercised observations. `mechanism`/`fallback` fields land alongside the mandatory schema as informative extras (not V1-mandatory). `cargo test --workspace` green, including the new real-evidence-store test (4.4a below).
- [x] 4.4a **(added, not in the original plan)** No test validated the real `docs/evidence/VP-001/{baselines,records}.md` files, only the synthetic fixtures. Added `tools/evidence-validator/tests/real_evidence_store.rs`. RED: filed the real baseline missing `test_date` — `cargo test -p evidence-validator --test real_evidence_store` failed with `field-missing`/`baseline-unpinned`, proving a malformed real record fails `cargo test --workspace`. GREEN: completed the baseline (and fixed one real V6 false positive: `tauri-apps/tauri-action@action-v1.0.0` reads as `user@host`-shaped; reworded to `tauri-apps/tauri-action, tag action-v1.0.0`) — test passes clean.
- [x] 4.6 Rollback boundary: a wrong row is corrected only by appending a new row referencing the original via `corrects`; never rewrite or delete it. Confirmed: reverting this commit removes both real records, the two retained transcript files, the new real-evidence-store test, and the README's `artifacts/` row layout entry; nothing downstream depends on this slice.

## Phase 5: Doc Reconciliation — Slice 5 (PR 5, independent, any position)

- [x] 5.1 `README.md` — update the status line (currently line 7, "design phase") to reflect the merged repository state once Slices 1–4 (or 1–3b/5 if authorization was declined) have landed. Done: replaced with a modest, accurate summary of the development-mode spike and a pointer to VP-001's evidence store.
- [x] 5.2 `docs/spike-log.md` — add a VP-001-appropriate entry reconciling this change. **Resolved** (parent's explicit direction superseded this task's own flagged ambiguity about design's `## Slice 5f` heading text): added `## Slice 5f — bounded local staging cleanup`, sourced only from `openspec/changes/archive/2026-09-14-spike-slice-5f/` (proposal, design, verify-report) and `openspec/specs/local-staging-cleanup/spec.md` — a real, already-merged and archived change this log had no entry for, distinct from this change's own design.md's (textually coincidental) `## Slice 5f` File Changes reference.
- [x] 5.3 Rollback boundary: revert both files; no code depends on their content. Confirmed.
