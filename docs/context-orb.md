...
**Interactive graph view (takeover surface).** Activating the idle orb plays a three-beat transition — particle implosion (~0.5 s, ease-in), hard white flash (~0.1 s), ease-out fade (~0.4 s) — into a full-screen concentric graph on the warm near-black ground, textured by the hexagonal mesh, which **continues past the applications band at application-cell size, fading out with radius** — those outer cells are the **application slot grid**: an empty cell is an unlit slot, an occupied cell lights up as the application, and the modeled region dissolves into space rather than ending at a border. From the center outward:
