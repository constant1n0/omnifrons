...
Debt, not drafted here. This document does not pin the exact OS build, architecture, WebView/runtime, packaging substrate, assistive-technology product, or toolchain version values themselves — each is a field this plan records only at first execution, never a default this draft assumes. The pointer from the target architecture's planned-assurance-artifacts row to this document was added by that table's first `Proposed` revision, accepted with the signed tag `accept/target-architecture/2026-09-05` (GOV-001-R16).

It does not define the CI-runner or manual-execution infrastructure that will actually host a scenario run — that is open decision D2, undecided here. It does not bootstrap the `docs/evidence/VP-001/` directory GOV-001's storage rule requires for this plan's own results — that directory does not yet exist. `docs/evidence/approvals.md`, the append-only approvals file GOV-001 declares a precondition for any `Accepted` transition, including VP-001's own, exists since 2026-09-04 (governance.md § Approval record and repository integrity ("repository's `main` branch is protected")).
