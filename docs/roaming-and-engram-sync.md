...
- Still owned by RSP-001 and not drafted here: workspace writer epochs, compare-and-set claim records, fencing, divergence detection, and recovery; and the Git Sync to Cloud cutover (quiescence, final watermark, backup, stable mutation IDs, reconciliation, fencing of the old authority, initial sync, commit point, tombstones, restore epochs, Git-history erasure limits, resurrection conflicts), shared with the migration and recovery plan (MRP-001).
