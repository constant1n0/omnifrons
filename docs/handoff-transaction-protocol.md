...
| HTP-001-R7 | Until the RSP-001 core's compare-and-set claim records are accepted, a second claim observed for the same handoff from a different receiver MUST render `conflicted` and MUST require human review; Omnifrons MUST NOT resolve it by last-writer-wins. |
